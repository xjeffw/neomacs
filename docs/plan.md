...
6. Expand queue-edge coverage for remaining prompt/input builtins (`read-key-sequence*`, `read-event`) against non-character and stale queue payloads.
7. Keep input-event queue edge cases for `read-char`/`read-key`/`read-key-sequence` aligned against oracle corpus (`invalid-event` and queue-empty paths).
8. Keep `check-all-neovm` as a recurring post-slice gate to catch regressions early.
